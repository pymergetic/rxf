...
The completed composed emitters publish executable mappings W^X: bytes are copied while writable, then switched to RX before the entry address is exposed. AArch64 firmware/QEMU images are linked into executable text before CPU start, so no runtime instruction-cache invalidation is required. A runtime that rewrites AArch64 text after CPU start must perform the platform cache-maintenance sequence before atomic publication.

The compiler QEMU proof is generated by `tools/generate_compiler_qemu.py` into `generated/compiler_qemu_aarch64.S`. That assembly contains the exact compiler-emitted checkout/main byte arrays and exact existing leaf bytes; `native/compiler_qemu_harness.c` only constructs transient ID slots, invokes the emitted entry, and reports `RXF-COMPOSED-PASS 570`.
